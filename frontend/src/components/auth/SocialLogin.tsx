...
export function SocialLoginButtons({
  ...
  const urls = useMemo(
    () => socialAuthUrls(SITE.apiBase, redirectUri || undefined),
    [redirectUri],
  );
  ...
  return (
    <div className={vertical ? "space-y-3" : "grid grid-cols-2 gap-3"}>
      <a
        href={googleUrl}
        className={`${blockClass} ${sizeClasses} inline-flex items-center justify-center gap-3 border-2 border-ink-950/15 bg-white text-ink-950 font-bold uppercase tracking-wide rounded-sm hover:border-ink-950/40 hover:-translate-y-0.5 transition`}
      >
      ...
      <a
        href={facebookUrl}
        className={`${blockClass} ${sizeClasses} inline-flex items-center justify-center gap-3 border-2 border-[#1877F2]/20 bg-[#1877F2] text-white font-bold uppercase tracking-wide rounded-sm hover:bg-[#166FE5] hover:-translate-y-0.5 transition`}
      >
