...
public class S3Helper {
    ...
    public void initializeS3Client() {
        if (accessKey == null || accessKey.isBlank() || secretKey == null || secretKey.isBlank()) {
            log.warn("AWS S3 credentials are not configured. S3 uploads will be disabled. Set AWS_ACCESS_KEY and AWS_SECRET_KEY env vars to enable.");
            this.s3Client = null;
            return;
        }
        try {
            ...
            log.error("Failed to initialize S3Client: {} — S3 uploads will be disabled. Set valid AWS_ACCESS_KEY / AWS_SECRET_KEY env vars to enable.", e.getMessage());
            this.s3Client = null;
        }
    }
    ...
    public String uploadImage(MultipartFile file, String folder) throws IOException {
        validateFile(file);
        if (s3Client == null) {
            throw new IOException("S3 is not configured — set AWS_ACCESS_KEY and AWS_SECRET_KEY env vars to enable file upload");
        }
    ...
    public boolean deleteFile(String fileUrl) {
        if (s3Client == null) {
            log.warn("S3 is not configured — cannot delete file: {}", fileUrl);
            return false;
        }
        try {
    ...
    public List<S3Object> listFiles(String folder) {
        if (s3Client == null) {
            log.warn("S3 is not configured — returning empty file list for folder: {}", folder);
